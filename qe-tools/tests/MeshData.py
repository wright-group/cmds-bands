...
def view_gradient(data):
    ...
    ax0.pcolormesh(data, channel=ch_i)
    ax0.contour(data, channel=ch_i)
    ax0.quiver(
    ...
    ax1.pcolormesh(data, channel=f"band{ch_i}_dx")

    ax2 = plt.subplot(gs[1, 0], sharex=ax0, sharey=ax0, aspect="equal")
    ax2.pcolormesh(data, channel=f"band{ch_i}_dy")

    ax3 = plt.subplot(gs[1, 1], sharex=ax0, sharey=ax0, aspect="equal")
    ax3.pcolormesh(data, channel=f"band{ch_i}_dz")
...
def view_JDOS(data):
    ...
    data.create_channel(name="diff43", values=data.band4[:] - data.band3[:], signed=False)
    data.grad(channel=-1)
